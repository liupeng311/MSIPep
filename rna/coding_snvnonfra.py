...
REFERENCE_PROTEOME = "./reference/Reference_protein_seq-RefSeq_ID.fasta"  # Replace with actual path
...
def handle_mutation(gene, chr, start, end, transcript_id, exonic_func, ref, alt, p_change, proteome_dict):
    ...
    if match_substitution:
        ...
        if pos > len(protein_seq):
            return fasta_records
        peptide = generate_peptide(protein_seq, pos)
        ...
        mutation_relative_start = pos1 - start_peptide_pos - 1
        mutation_relative_end = pos2 - start_peptide_pos
        ...
        try:
            ...
            end_peptide_pos = pos2 + 30
            peptide = protein_seq[start_peptide_pos:end_peptide_pos]
            insertion_relative_pos = pos2 - start_peptide_pos
            if insertion_relative_pos < 0 or insertion_relative_pos > len(peptide):
                return fasta_records
            expected_ref_aa = protein_seq[pos2 - 1]
            actual_ref_aa = '' if insertion_relative_pos == 0 else peptide[insertion_relative_pos - 1]
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python script2.py <annovar_annotation_file> <output_fasta>")
        sys.exit(1)
    ...
    print("Reading ANNOVAR annotation file...")
    df = pd.read_csv(annovar_file, sep='\t', header=0)

    if 'AAChange.refGene' not in df.columns:
        print("Error: Missing 'AAChange.refGene' column in annotation file. Please check file format.")
        sys.exit(1)
    ...
    with open(output_fasta, 'w') as f:
        for record in fasta_records:
            f.write(record + '\n')
    # print("Output saved to", output_fasta)
