...
def process_file(input_file, output_dir, tools):
    input_dir = os.path.dirname(os.path.abspath(input_file))
    base_name = os.path.splitext(os.path.basename(input_file))[0]
    msconvert_sif = tools["msconvert_sif"]
    pepnet_python = tools["pepnet_python"]
    pepnet_script = tools["pepnet_script"]
    pepnet_model = tools["pepnet_model"]

    if input_file.lower().endswith(".raw"):
        # Step 1: Convert raw -> mgf
        run_command(
            f"singularity exec --cleanenv --bind {input_dir}:/data {msconvert_sif} "
    ...
    run_command(
        f"{pepnet_python} {pepnet_script} "
        f"--input {charged_mgf} --model {pepnet_model} "
        f"--output {tsv_path}"
    )
...
def main():
    ...
    parser.add_argument("--output_fasta", type=str, default="", help="Path to output merged FASTA file.")
    parser.add_argument(
        "--pepnet-only",
        action="store_true",
        help="Run msconvert + PepNet only; output TSV without built-in filtering or FASTA merge (filtering recommended via 2.pepnet-filter.py)",
    )
    parser.add_argument("--msconvert_sif", default="./software/msconvert.sif", help="Path to msconvert Singularity image")
    parser.add_argument("--pepnet_script", default="./software/PepNet-master/denovo.py", help="Path to PepNet denovo.py")
    parser.add_argument("--pepnet_model", default="./software/PepNet-master/model.h5", help="Path to PepNet model")
    parser.add_argument("--pepnet_python", default="python", help="Python interpreter for running PepNet")
    ...
    tools = {
        "msconvert_sif": args.msconvert_sif,
        "pepnet_script": args.pepnet_script,
        "pepnet_model": args.pepnet_model,
        "pepnet_python": args.pepnet_python,
    }

    # Step 1-3: Process each raw/mgf file
    for file in input_files:
        process_file(file, args.output_dir, tools)

    if args.pepnet_only:
        print(f"✅ PepNet complete, TSV output directory: {args.output_dir}")
        return

    if not args.output_fasta:
        sys.exit("❌ --output_fasta not specified; use --pepnet-only if you only need PepNet TSV")

    # Step 4: Filter all TSVs and merge to one FASTA (legacy built-in filter; prefer 2.pepnet-filter.py)
    all_sequences = []
