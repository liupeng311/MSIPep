#!/usr/bin/env python3
"""
RNA-seq Somatic Mutation Calling Pipeline——MAF
================================================
Goal: Use tumor RNA-seq data only, apply strict filtering, and output a somatic mutation MAF file suitable for neoantigen prediction.
...
def resolve_trimmomatic_paired_outputs(base_with_ext: str):
    """
    Trimmomatic PE -baseout naming may vary slightly across environments;
    try common patterns and prefer paired outputs (1P/2P).
    """
    base_no_fastq_gz = (
        base_with_ext[:-9] if base_with_ext.lower().endswith(".fastq.gz") else base_with_ext
    )
    candidates = [
        # Common Trimmomatic -baseout xxx.fastq.gz output names
        (f"{base_no_fastq_gz}_1P.fastq.gz", f"{base_no_fastq_gz}_2P.fastq.gz"),
        (f"{base_no_fastq_gz}_1P", f"{base_no_fastq_gz}_2P"),
        # Legacy / alternate naming
        (f"{base_with_ext}_1P", f"{base_with_ext}_2P"),
        (f"{base_with_ext}_1P.fastq.gz", f"{base_with_ext}_2P.fastq.gz"),
    ]
    for fq1, fq2 in candidates:
        if file_exists_and_nonempty(fq1) and file_exists_and_nonempty(fq2):
            return fq1, fq2
    raise FileNotFoundError(
        f"Trimmomatic paired-end outputs (1P/2P) not found. Checked: {candidates}"
    )
...
def main(args):
    ...
    fq2     = validate_fastq_input(args.fq2, "--fq2")
    ...
    trimmomatic_output_base = os.path.join(out_dir, f"{sample}_trimmomatic")
    trimmomatic_base_with_ext = f"{trimmomatic_output_base}.fastq.gz"
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 0. Trimmomatic preprocessing (paired-end)
    # ════════════════════════════════════════════════════════════════════════
    trimmed_outputs_ready = True
    try:
        resolve_trimmomatic_paired_outputs(trimmomatic_base_with_ext)
    except FileNotFoundError:
        trimmed_outputs_ready = False

    if not trimmed_outputs_ready:
        trimmomatic_command = (
        ...
        run_command(trimmomatic_command, "Trimmomatic PE")
    else:
        logger.info("⏭ Skip (exists): Trimmomatic PE")

    fq1, fq2 = resolve_trimmomatic_paired_outputs(trimmomatic_base_with_ext)
    logger.info(f"Continuing with Trimmomatic outputs: {fq1}, {fq2}")
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 1. STAR 2-pass alignment (embedded RG info, required by Mutect2)
    # ════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 3. GATK SplitNCigarReads (RNA-specific: handle spliced N-CIGAR)
    # ════════════════════════════════════════════════════════════════════════
    if not should_skip_step(latest, 3, split_bam, "SplitNCigarReads"):
        run_command(
            ...
            f"-I {marked_bam} "
            f"-O {split_bam} "
            f"--skip-mq-transform false",   # keep MQ transform for better RNA alignment QC
            "SplitNCigarReads"
        )

    # ════════════════════════════════════════════════════════════════════════
    # Step 4. BQSR base quality recalibration
    # ════════════════════════════════════════════════════════════════════════
    if not should_skip_step(latest, 4, recal_table, "BaseRecalibrator"):
        run_command(
            ...
            f"-I {split_bam} "
            f"--known-sites {dbsnp} "
            f"--known-sites {gnomad} "   # also use gnomAD as known sites to reduce over-correction
            f"-O {recal_table}",
            "Base Recalibration"
        )
    ...
    #   - Fix: use --tumor-sample instead of deprecated -tumor
    #   - Added: --f1r2-tar-gz for orientation bias (used in Step 6)
    #   - Recommended: add PoN to greatly reduce tumor-only false positives
    # ════════════════════════════════════════════════════════════════════════
    ...
    if skip_mutect2 and not all_files_exist(mutect2_expected_outputs):
        logger.warning(
            "⚠ Mutect2 key outputs incomplete (missing raw_vcf/raw_vcf_stats/f1r2); "
            "will rerun Step 5 Mutect2."
        )
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 6. Learn read orientation bias model (F1R2, for FFPE / oxidation damage)
    # ════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 7. Contamination estimation (core false-positive control in tumor-only mode)
    # ════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 8. FilterMutectCalls (orientation model + contamination + stats)
    # ════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 9. Extract PASS variants (FilterMutectCalls does not remove non-PASS records)
    # ════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 10. RNA editing site filter (REDIportal only)
    #   Rationale: A→I editing (appears as A→G in RNA-seq) is the largest FP source.
    #   Note: RADAR database omitted due to download failure; REDIportal filter only.
    # ════════════════════════════════════════════════════════════════════════
    # Decompress pass_vcf for bedtools (bedtools -header does not support bgzipped VCF directly)
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Step 11. Write clean_vcf path for outer Bash script to run vcf2maf
    #   vcf2maf needs VEP in a separate vep conda env;
    #   env switching is unreliable inside Python subprocess,
    #   so run_pipeline.sh handles env switch and vcf2maf.
    # ════════════════════════════════════════════════════════════════════════
    ...
    # Final cleanup: delete intermediates only after final output exists
    ...
    logger.info("=" * 60)
    logger.info(f"✅ Python pipeline complete (Steps 1–10)!")
    logger.info(f"   clean_vcf : {clean_vcf}")
    logger.info(f"   Next: run_pipeline.sh switches to vep env for vcf2maf annotation")
    logger.info("=" * 60)

# ── Argument parsing ──────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RNA-seq Somatic Mutation_ MAF Pipeline v2.3 | Step 1-10 | invoked by run_pipeline.sh",
    ...
    parser.add_argument(
        "--fq1",
        required=True,
        help="FASTQ R1 (.fastq/.fq/.fastq.gz/.fq.gz/.gz, also .fsatq.gz)"
    )
    parser.add_argument(
        "--fq2",
        required=True,
        help="FASTQ R2 (.fastq/.fq/.fastq.gz/.fq.gz/.gz, also .fsatq.gz)"
    )
